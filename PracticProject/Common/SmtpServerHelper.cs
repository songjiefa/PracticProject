...
namespace Common
{
	...
	public class MailAttachments
	{
		...
		public string this[int index] => (string) _attachments[index];
		...
		///     添加邮件附件
		/// </summary>
		/// <param name="i_filePath">附件的绝对路径</param>
		public void Add(params string[] i_filePath)
		{
			if (i_filePath == null)
				throw new ArgumentNullException("非法的附件");
			foreach (var file in i_filePath)
				Add(file);
		}
	...
	public class MailMessage
	{
		...
		{
			if (Recipients.Count < MaxRecipientNum) Recipients.Add(recipient);
		}

		/// <summary>
		///     增加多个收件人地址
		/// </summary>
		/// <param name="i_recipient">收件人的Email地址集合</param>
		public void AddRecipients(params string[] i_recipient)
		{
			if (i_recipient == null)
				throw new ArgumentException("收件人不能为空.");
			foreach (var recipient in i_recipient)
	...
	public class SmtpServerHelper
	{
		...
		///     将字符串编码为Base64字符串
		/// </summary>
		/// <param name="i_str">要编码的字符串</param>
		private string Base64Encode(string i_str)
		{
			var barray = Encoding.Default.GetBytes(i_str);
		...
		private bool Dialog(string str, string errstr)
		{
			...
			if (SendCommand(str))
			{
				...
				if (_rightCodeHt[rrCode] != null) return true;
		...
		private string GetPriorityString(MailPriority mailPriority)
		{
			...
			if (mailPriority == MailPriority.Low)
				priority = "Low";
			else if (mailPriority == MailPriority.High) priority = "High";
			return priority;
		}
			...
			MailMessage mailMessage)
		{
			...
			if (html)
			{
				...
				sendBufferstr += (mailMessage.Charset == ""
					                 ? " charset=\"iso-8859-1\""
					                 : " charset=\"" + mailMessage.Charset.ToLower() + "\"") + _crlf;
			...
			{
				if (mailMessage.Attachments.Count != 0) sendBufferstr += "--=====001_Dragon303406132050_=====" + _crlf;
				sendBufferstr += "Content-Type: text/plain;" + _crlf;
				sendBufferstr += (mailMessage.Charset == ""
					                 ? " charset=\"iso-8859-1\""
					                 : " charset=\"" + mailMessage.Charset.ToLower() + "\"") + _crlf;
			...
			if (mailMessage.Attachments.Count != 0)
			{
				for (var i = 0; i < mailMessage.Attachments.Count; i++)
				{
					...
					sendBufferstr += " name=\"=?" + mailMessage.Charset.ToUpper() + "?B?" +
					                 Base64Encode(filepath.Substring(filepath.LastIndexOf("\\") + 1)) + "?=\"" + _crlf;
	...
	public class SmtpClient
	{
		public bool Send(MailMessage mailMessage, string username, string password)
		{
			...
			if (helper.SendEmail(SmtpServer, 25, username, password, mailMessage)) return true;
